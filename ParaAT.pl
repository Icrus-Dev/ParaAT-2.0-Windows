...
#Operating system
my $OS = "$^O\n";
if (rindex($OS, "linux", 0)==0) {
	$Seperator = '/';
	$EnvSeperator = ":";
	$Extension = '';
}
elsif (rindex($OS, "MSWin32", 0)==0) {
	$Seperator = '\\';
	$EnvSeperator = ";";
	$Extension = '.exe';
	$PWD =~ s/\//$Seperator/g;
	...
	print "Not supported: Operating system (".$OS.") \n";
	exit(1);
}
...
if (CheckCMD($Pal2NalCMD)==1) {
    ...
    print "[NA]\n";
    print "Please make sure Epal2nal.pl is executable (+X) and put into the global directory.\n";
    exit(0);
}
...
if ($KaKs_Calculator) {
    ...
    if (CheckCMD($KaKsCMD.$Extension)==0) {
...
if (-e $OutputFolder) {
	...
	$OutputFolder = $OutputFolder.$Seperator;
	mkdir($OutputFolder);
}
print "[OK]\n";
...
sub CheckCMD {
	...
	if (-x $cmd) {
		...
		# my @PATH = undef;
		my @PATH = split($EnvSeperator, $string_path);
        chomp @PATH;
        ...
        for ($i=0; $i<@PATH && $Exist==0; ) {
            ...
            if (-e $PATH[$i].$Seperator.$cmd) {
				if (rindex($OS,'MSWin32',0)==0 || (rindex($OS,'linux',0)==0 && -x $PATH[$i].$Seperator.$cmd)) {
					$Exist = 1;
				}
            }
            else {
...
sub GenerateSequenceFiles(){
	...
	for (my $i=0; $i<@homologs; $i++) {
		...
		if ($flag) {
			open(OUTFILE, ">$OutputFolder$file.pep");
	  		print OUTFILE ("$aa_seq");
	  
	  		open(OUTFILE, ">$OutputFolder$file.cds");
	  		print OUTFILE ("$nuc_seq");
	  
	  		push(@files, $OutputFolder.$file);
	  	}
	  	else {
...
sub AlignProteinSeqs(){
	...
	if ($MSACMD =~ /clustalw2/) {
	    $cmd = "clustalw2".$Extension." -INFILE=".$file.".pep  -PWMATRIX=BLOSUM -TYPE=PROTEIN -OUTPUT=FASTA -OUTFILE=".$file.".pep_aln"; #  >> msg.msa";
	}
	elsif ($MSACMD =~ /mafft/) {
	    $cmd = 	"mafft".$Extension."  --auto --quiet  ".$file.".pep  >  ".$file.".pep_aln";
	}
	elsif ($MSACMD =~ /muscle/) {
	    $cmd = "muscle".$Extension." -quiet -align ".$file.".pep -output ".$file.".pep_aln"; #  >> msg.msa";
	}
	elsif ($MSACMD =~ /t_coffee/) { # not stable and greedy for memory, tested on MAC OSX
        $cmd = 	"t_coffee".$Extension."  ".$file.".pep  -output=fasta  -outfile=".$file.".pep_aln -quiet"; #  >> msg.msa";
	}
	return $cmd;
}
...
sub PA2CACommands {
	...
	for (my $i=0; $i<@files; $i++) {
		my $file = $files[$i];
		my $cmd = "perl ".$Pal2NalCMD."  ".$file.".pep_aln  ".$file.".cds  -output $OutputFormat  $para > ".$file.".cds_aln.".$OutputFormat;
		push(@cmds, $cmd);
	} 
	chomp @cmds;
	return @cmds;
...
sub KaKsCommands {
	...
	for (my $i=0; $i<@files; $i++) {
		my $file = $files[$i];
		my $cmd = $KaKsCMD.$Extension." -i ".$file.".cds_aln.axt -o  ".$file.".cds_aln.axt.kaks -m MA -c ".$Code.""; #  >> msg.kaks";
		push(@cmds, $cmd);
	} 
	chomp @cmds;
	return @cmds;
